  ...
  async execute(ctx: AgentRunContext): Promise<AgentResult> {
    ...
    const caseRow = db.prepare(`
      SELECT sla_first_response_deadline, sla_resolution_deadline,
             first_response_at, assigned_user_id, assigned_team_id
    ...
    const stalledApprovals = db.prepare(`
      SELECT id, action_type, created_at FROM approval_requests
    ...
    for (const approval of stalledApprovals) {
      ...
      if (hoursWaiting > 8) {
        events.push({
          ...
          severity: 'critical',
          detail: `Approval ${approval.id} (${approval.action_type}) pending for ${Math.round(hoursWaiting)}h`,
