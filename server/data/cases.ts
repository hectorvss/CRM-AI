...
async function listCasesSupabase(scope: CaseScope, filters: CaseFilters) {
  ...
  const [customersRes, usersRes, teamsRes, messagesRes, ordersRes, paymentsRes, returnsRes, refundsRes] = await Promise.all([
    ...
    orderIds.length ? supabase.from('orders').select('id, case_id, status, fulfillment_status, external_order_id, updated_at, has_conflict, conflict_domain, conflict_detected, risk_level').in('id', orderIds).eq('tenant_id', scope.tenantId) : Promise.resolve({ data: [], error: null } as any),
    paymentIds.length ? supabase.from('payments').select('id, order_id, customer_id, status, approval_status, refund_status, external_payment_id, updated_at, has_conflict, conflict_detected, risk_level').in('id', paymentIds).eq('tenant_id', scope.tenantId) : Promise.resolve({ data: [], error: null } as any),
