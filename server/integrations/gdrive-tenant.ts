/**
 * server/integrations/gdrive-tenant.ts
 *
 * Per-tenant Google Drive adapter resolver. Uses shared google-oauth refresh.
 */
...
interface CacheEntry { adapter: GDriveAdapter; cachedAt: number; expiresAt: number }
const cache = new Map<string, CacheEntry>();
const cacheKey = (t: string, w: string | null) => `${t}::${w ?? '_'}`;

export interface GDriveChannelEntry {
  channel_id: string;
  resource_id: string;
  page_token: string;
  token: string;
...
export async function loadGDriveConnector(tenantId: string): Promise<GDriveConnector | null> {
  try {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.from('connectors').select('id, tenant_id, system, status, auth_config').eq('tenant_id', tenantId).eq('system', 'gdrive').eq('status', 'connected').order('updated_at', { ascending: false }).limit(1).maybeSingle();
    ...
    return {
      ...
      channels: Array.isArray(cfg.channels) ? cfg.channels as GDriveChannelEntry[] : [],
      rawAuthConfig: cfg,
    };
  } catch (err) { logger.warn('loadGDriveConnector failed', { tenantId, error: String(err) }); return null; }
}

export async function findTenantByGDriveChannelToken(token: string): Promise<{ tenantId: string; connectorId: string; channel: GDriveChannelEntry } | null> {
  if (!token) return null;
  try {
    const supabase = getSupabaseAdmin();
    const { data } = await supabase.from('connectors').select('id, tenant_id, auth_config').eq('system', 'gdrive').eq('status', 'connected');
    if (!data) return null;
    for (const row of data) {
      const cfg = (row.auth_config ?? {}) as Record<string, unknown>;
      const channels = Array.isArray(cfg.channels) ? cfg.channels as GDriveChannelEntry[] : [];
    ...
    return null;
  } catch (err) { logger.warn('findTenantByGDriveChannelToken failed', { error: String(err) }); return null; }
}

async function refreshIfNeeded(connector: GDriveConnector): Promise<GDriveConnector> {
  ...
  } catch (err) { logger.warn('gdrive token refresh failed', { connectorId: connector.id, error: String(err) }); return connector; }
}

export async function gdriveForTenant(tenantId: string, workspaceId: string | null): Promise<{ adapter: GDriveAdapter; connector: GDriveConnector } | null> {
  const key = cacheKey(tenantId, workspaceId);
  const hit = cache.get(key);
  if (hit && Date.now() < hit.expiresAt - 60_000 && Date.now() - hit.cachedAt < 5 * 60_000) {
    const connector = await loadGDriveConnector(tenantId);
    if (connector) return { adapter: hit.adapter, connector };
  }
  let connector = await loadGDriveConnector(tenantId);
  if (!connector) { cache.delete(key); return null; }
  connector = await refreshIfNeeded(connector);
  const adapter = new GDriveAdapter(connector.accessToken);
...
export function invalidateGDriveForTenant(tenantId: string, workspaceId: string | null): void {
