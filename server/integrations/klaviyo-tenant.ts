/**
 * server/integrations/klaviyo-tenant.ts
 */

import { getSupabaseAdmin } from '../db/supabase.js';
import { logger } from '../utils/logger.js';
import { KlaviyoAdapter } from './klaviyo.js';
import { refreshAccessToken, type KlaviyoOAuthEnv } from './klaviyo-oauth.js';

interface CacheEntry { adapter: KlaviyoAdapter; cachedAt: number; expiresAt: number }
const cache = new Map<string, CacheEntry>();
const cacheKey = (t: string, w: string | null) => `${t}::${w ?? '_'}`;

export interface KlaviyoWebhookEntry { webhook_id: string; url: string; topics: string[]; secret: string }

export interface KlaviyoConnector {
  ...
  webhooks: KlaviyoWebhookEntry[];
  rawAuthConfig: Record<string, unknown>;
}

function readEnv(): KlaviyoOAuthEnv | null {
...
export async function loadKlaviyoConnector(tenantId: string): Promise<KlaviyoConnector | null> {
  try {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.from('connectors').select('id, tenant_id, system, status, auth_config').eq('tenant_id', tenantId).eq('system', 'klaviyo').eq('status', 'connected').order('updated_at', { ascending: false }).limit(1).maybeSingle();
    ...
    return {
      ...
      webhooks: Array.isArray(cfg.webhooks) ? cfg.webhooks as KlaviyoWebhookEntry[] : [],
      rawAuthConfig: cfg,
    };
  } catch (err) { logger.warn('loadKlaviyoConnector failed', { tenantId, error: String(err) }); return null; }
}

export async function findKlaviyoTenantBySignature(verify: (secret: string) => boolean): Promise<{ tenantId: string; connectorId: string; webhook: KlaviyoWebhookEntry } | null> {
  try {
    const supabase = getSupabaseAdmin();
    const { data } = await supabase.from('connectors').select('id, tenant_id, auth_config').eq('system', 'klaviyo').eq('status', 'connected');
    if (!data) return null;
    for (const row of data) {
      const cfg = (row.auth_config ?? {}) as Record<string, unknown>;
      const webhooks = Array.isArray(cfg.webhooks) ? cfg.webhooks as KlaviyoWebhookEntry[] : [];
      for (const wh of webhooks) {
        if (verify(wh.secret)) return { tenantId: String(row.tenant_id), connectorId: String(row.id), webhook: wh };
      }
    }
    return null;
  } catch (err) { logger.warn('findKlaviyoTenantBySignature failed', { error: String(err) }); return null; }
}

async function refreshIfNeeded(connector: KlaviyoConnector): Promise<KlaviyoConnector> {
  ...
  try {
    ...
    const supabase = getSupabaseAdmin();
    const merged = { ...connector.rawAuthConfig, access_token: grant.accessToken, refresh_token: grant.refreshToken ?? connector.refreshToken, scope: grant.scope || connector.scope, access_token_expires_at: new Date(Date.now() + grant.expiresIn * 1000).toISOString() };
  ...
  } catch (err) { logger.warn('klaviyo token refresh failed', { connectorId: connector.id, error: String(err) }); return connector; }
}

export async function klaviyoForTenant(tenantId: string, workspaceId: string | null): Promise<{ adapter: KlaviyoAdapter; connector: KlaviyoConnector } | null> {
  const key = cacheKey(tenantId, workspaceId);
  const hit = cache.get(key);
  if (hit && Date.now() < hit.expiresAt - 60_000 && Date.now() - hit.cachedAt < 5 * 60_000) {
    const connector = await loadKlaviyoConnector(tenantId);
    if (connector) return { adapter: hit.adapter, connector };
  }
  let connector = await loadKlaviyoConnector(tenantId);
  if (!connector) { cache.delete(key); return null; }
  connector = await refreshIfNeeded(connector);
  const adapter = new KlaviyoAdapter(connector.accessToken);
...
export function invalidateKlaviyoForTenant(tenantId: string, workspaceId: string | null): void {
