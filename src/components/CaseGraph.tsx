...
export default function CaseGraph({ onPageChange }: { onPageChange: (page: Page) => void }) {
  ...
  const branches: GraphBranch[] = useMemo(() => {
    const source = caseGraph?.branches || [];
    return source.map((branch: any) => ({
      id: branch.id,
      label: branch.label,
      icon: iconForBranch(branch.id),
      page: pageForBranch(branch.id),
      status: graphStatus(branch.status),
      nodes: (branch.nodes || []).map((node: any) => ({
        id: node.id,
        label: node.label,
        status: graphStatus(node.status),
        context: node.context || node.value || '',
        icon: iconForBranch(branch.id),
  ...
  const rootData = useMemo(() => ({
    orderId: caseGraph?.root?.order_id || caseState?.identifiers?.order_ids?.[0] || caseState?.identifiers?.case_number || 'Case',
    customerName: caseGraph?.root?.customer_name || caseState?.customer?.canonical_name || 'Unknown',
    riskLevel: formatStatus(caseGraph?.root?.risk_level || caseState?.case?.risk_level || 'low'),
  }), [caseGraph, caseState]);

  const timeline = caseGraph?.timeline || [];
  const impactedBranches = branches.filter(branch => branch.status !== 'healthy');
  const links = (caseState?.systems?.integrations?.identifiers || []).map((value: string) => ({
    label: value,
    href: '#',
  }));
  const relatedCases = caseResolve?.linked_cases || [];
  const internalNotes = caseResolve?.notes || [];
