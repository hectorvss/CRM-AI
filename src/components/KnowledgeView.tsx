          ...
          <div className="bg-white dark:bg-card-dark rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
            ...
            <div className="p-8 space-y-12">
              ...
              <section>
                ...
                <div className="space-y-6">{profile.source_categories.map((category, idx) => <div key={idx} className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden"><div className="bg-gray-50 dark:bg-gray-800/50 px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between"><h4 className="text-sm font-bold text-gray-700 dark:text-gray-300">{category.name}</h4><span className="text-xs text-gray-500">{category.sources.length} sources</span></div><div className="divide-y divide-gray-100 dark:divide-gray-800">{category.sources.map((source, sIdx) => { const level = profile.source_access[source] || 'No access'; return <div key={sIdx} className="bg-white dark:bg-card-dark px-4 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors"><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{source}</span><select value={level} onChange={(e) => setProfile(prev => ({ ...prev, source_access: { ...prev.source_access, [source]: e.target.value as AccessLevel } }))} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>No access</option><option>Metadata only</option><option>Read summaries only</option><option>Read raw documents</option><option>Read + extract</option><option>Approval required</option></select></div>; })}</div></div>)}</div>
              </section>

              <section className="grid grid-cols-2 gap-8">
                <div className="space-y-4"><label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Document Status</label><select value={profile.document_status} onChange={(e) => setProfile(prev => ({ ...prev, document_status: e.target.value }))} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Final documents only</option><option>Include drafts</option><option>Approved policies only</option></select><div className="space-y-2 mt-2"><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.include_internal_notes} onChange={() => setProfile(prev => ({ ...prev, include_internal_notes: !prev.include_internal_notes }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Internal notes visible</label><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.include_attachments} onChange={() => setProfile(prev => ({ ...prev, include_attachments: !prev.include_attachments }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Attachments allowed</label><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.include_historical_conversations} onChange={() => setProfile(prev => ({ ...prev, include_historical_conversations: !prev.include_historical_conversations }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Historical conversations allowed</label></div></div>
                <div className="space-y-4"><label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Archived Records</label><select value={profile.archived_records} onChange={(e) => setProfile(prev => ({ ...prev, archived_records: e.target.value }))} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Allowed</option><option>Blocked</option><option>Metadata only</option></select><div className="space-y-2 mt-2"><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.block_admin_notes} onChange={() => setProfile(prev => ({ ...prev, block_admin_notes: !prev.block_admin_notes }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Admin-only notes blocked</label><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.block_finance_docs} onChange={() => setProfile(prev => ({ ...prev, block_finance_docs: !prev.block_finance_docs }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Finance-only documents blocked</label><label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={profile.block_legal_docs} onChange={() => setProfile(prev => ({ ...prev, block_legal_docs: !prev.block_legal_docs }))} className="rounded text-indigo-600 focus:ring-indigo-500" />Legal documents restricted</label></div></div>
              </section>

              <section>
                <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Sensitive Data Restrictions</h3>
                <div className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden"><div className="divide-y divide-gray-100 dark:divide-gray-800">{profile.sensitive_data_types.map((type, idx) => { const rule = profile.sensitive_rules[type] || 'Hidden completely'; return <div key={idx} className="bg-white dark:bg-card-dark px-4 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors"><div className="flex items-center gap-3"><span className="material-symbols-outlined text-gray-400 text-sm">security</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{type}</span></div><select value={rule} onChange={(e) => setProfile(prev => ({ ...prev, sensitive_rules: { ...prev.sensitive_rules, [type]: e.target.value as SensitiveRule } }))} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Hidden completely</option><option>Masked</option><option>Summary only</option><option>View with approval</option><option>Never accessible</option></select></div>; })}</div></div>
              </section>

              <section className="grid grid-cols-2 gap-8">
                <div><label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Search Depth</label><select value={profile.search_depth} onChange={(e) => setProfile(prev => ({ ...prev, search_depth: e.target.value }))} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Only directly linked sources</option><option>Same-case data only</option><option>Related documents allowed</option><option>Cross-case lookup allowed</option></select></div>
                <div><label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Historical Lookup Depth</label><select value={profile.historical_lookup_depth} onChange={(e) => setProfile(prev => ({ ...prev, historical_lookup_depth: e.target.value }))} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Last 30 days</option><option>Last 6 months</option><option>Last 1 year</option><option>All time</option></select></div>
              </section>
              ...
              <section>
                ...
                <div className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden"><div className="divide-y divide-gray-100 dark:divide-gray-800">{profile.field_visibility_items.map((field, idx) => { const visibility = profile.field_visibility[field] || 'Hidden'; return <div key={idx} className="bg-white dark:bg-card-dark px-4 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors"><div className="flex items-center gap-3"><span className="material-symbols-outlined text-gray-400 text-sm">data_object</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{field}</span></div><select value={visibility} onChange={(e) => setProfile(prev => ({ ...prev, field_visibility: { ...prev.field_visibility, [field]: e.target.value as FieldVisibility } }))} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><option>Visible</option><option>Masked</option><option>Summary only</option><option>Hidden</option><option>Approval required</option></select></div>; })}</div></div>
              </section>
