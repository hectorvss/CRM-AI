        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700">
            {['All', 'Active', 'Restricted', 'Draft'].map(f => (
              <button key={f} onClick={() => setFilter(f as 'All' | 'Active' | 'Restricted' | 'Draft')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${filter === f ? 'bg-black dark:bg-white text-white dark:text-black shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>{f}</button>
            ))}
          </div>
        </div>
            ...
            <div className="p-6 space-y-8">
              ...
              <section className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">{['No formal verification', 'Light verification', 'Moderate verification', 'Strict verification'].map(value => <label key={value} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><input type="radio" name="verificationBehavior" checked={agentConfig.verificationBehavior === value} onChange={() => setAgentConfig(prev => ({ ...prev, verificationBehavior: value as AgentReasoningConfig['verificationBehavior'] }))} className="text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300">{value}</span></label>)}</div>
                </div>
              </section>
              ...
              <section className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">{['One trusted source', 'One source + contextual confirmation', 'Multiple sources when available', 'Mandatory multi-source agreement'].map(value => <label key={value} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><input type="radio" name="multiSourceCrossChecking" checked={agentConfig.multiSourceCrossChecking === value} onChange={() => setAgentConfig(prev => ({ ...prev, multiSourceCrossChecking: value as AgentReasoningConfig['multiSourceCrossChecking'] }))} className="text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300">{value}</span></label>)}</div>
                </div>
              </section>
              ...
              <section className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">{['Proceed with best-effort judgment', 'Proceed only in low-risk cases', 'Respond with caveats', 'Request more context first', 'Defer decision if confidence is low', 'Avoid action under ambiguity'].map(value => <label key={value} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><input type="radio" name="uncertaintyHandling" checked={agentConfig.uncertaintyHandling === value} onChange={() => setAgentConfig(prev => ({ ...prev, uncertaintyHandling: value as AgentReasoningConfig['uncertaintyHandling'] }))} className="text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300">{value}</span></label>)}</div>
                </div>
                <div>
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Decision Strictness</h3>
                  <div className="space-y-2">{['Conservative', 'Balanced', 'Assertive', 'Strict-compliance-first', 'Heuristic / flexible'].map(value => <label key={value} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><input type="radio" name="decisionStrictness" checked={agentConfig.decisionStrictness === value} onChange={() => setAgentConfig(prev => ({ ...prev, decisionStrictness: value as AgentReasoningConfig['decisionStrictness'] }))} className="text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300">{value}</span></label>)}</div>
                </div>
              </section>
              ...
              <section>
                <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Response Construction Logic</h3>
                  <div className="grid grid-cols-3 gap-3">{['Concise outcome only', 'Outcome + reasoning summary', 'Outcome + evidence references', 'Structured decision explanation', 'Decision + confidence signal', 'Recommendation + uncertainties'].map(value => <label key={value} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><input type="radio" name="responseConstructionLogic" checked={agentConfig.responseConstructionLogic === value} onChange={() => setAgentConfig(prev => ({ ...prev, responseConstructionLogic: value as AgentReasoningConfig['responseConstructionLogic'] }))} className="text-indigo-600 focus:ring-indigo-500" /><span className="text-sm font-medium text-gray-700 dark:text-gray-300">{value}</span></label>)}</div>
              </section>
