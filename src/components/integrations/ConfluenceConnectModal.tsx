...
const SCOPES = [
  { id: 'spaces',   label: 'Spaces · structure',                 icon: 'folder' },
  { id: 'pages',    label: 'Pages · body en storage / view',     icon: 'description' },
  { id: 'search',   label: 'CQL search · full-text',             icon: 'search' },
  { id: 'users',    label: 'Users · accountId resolution',       icon: 'group' },
  { id: 'rag',      label: 'RAG-ready · chunking en pipeline',   icon: 'graph_2' },
  { id: 'refresh',  label: 'OAuth refresh transparente (60s)',   icon: 'autorenew' },
];

const ConfluenceConnectModal: React.FC<Props> = ({ open, onClose, onChanged, existing }) => {
  ...
  async function handleInstall() {
    ...
    try {
      const res = await authedFetch('/api/integrations/confluence/install');
      if (!res.ok) {
  ...
  async function handleSync() {
    ...
    try {
      const res = await authedFetch('/api/integrations/confluence/sync', { method: 'POST' });
      const j = await res.json();
  ...
  async function handleDisconnect() {
    if (!confirm('¿Desconectar Confluence?')) return;
    setSubmitting(true);
    try {
      const res = await authedFetch('/api/integrations/confluence/disconnect', { method: 'POST' });
        ...
        <div className="flex items-start justify-between gap-4 border-b border-black/5 px-6 py-5 dark:border-white/10">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#0052CC] text-white shadow-sm">
              <IntegrationLogo id="confluence" size={22} />
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400">Knowledge · Atlassian</p>
              <h2 className="text-lg font-semibold text-gray-950 dark:text-white">Confluence</h2>
              <p className="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">Knowledge base · pages como contexto RAG · CQL search</p>
            </div>
          </div>
        ...
        <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
          {isConnected ? (
            <>
              ...
              <div className="rounded-2xl border border-black/5 bg-gray-50/50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => void handleSync()} disabled={syncStatus === 'syncing'} className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-[11px] font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-50 dark:border-white/10 dark:bg-[#171717] dark:text-gray-200 dark:hover:bg-white/5">
                    {syncStatus === 'syncing' ? 'Sincronizando…' : syncStatus === 'ok' ? '✓ OK' : syncStatus === 'error' ? '⚠ Error' : 'Pages recientes'}
                  </button>
                </div>
                {syncResult ? (
                  <div className="mt-2 text-[11px] text-emerald-700 dark:text-emerald-300">
                    {syncResult.pages_visible} pages.
                    {syncResult.sample?.length ? (
                      <ul className="mt-1 space-y-0.5">
                        {syncResult.sample.map((p: any) => (
                          <li key={p.id} className="truncate"><span className="font-medium">{p.id}</span> · {p.status} · {p.title}</li>
                        ))}
                      </ul>
                    ) : null}
                  </div>
              ...
              <div className="mt-3 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-[11px] text-blue-800 dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-200">
                <p className="font-semibold uppercase tracking-[0.18em]">Knowledge ingest</p>
                <p className="mt-1 leading-relaxed">
                  Confluence Cloud no expone webhooks dinámicos vía OAuth 3LO. Indexamos en pipeline polling cada 6h vía <code>list pages</code> + <code>get page body</code> en formato storage/view. El AI consulta con CQL.
                </p>
              </div>
            ...
            <>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Conexión vía OAuth 3LO de Atlassian. El AI agent indexa tus pages como contexto RAG: cuando un cliente pregunta algo, el agent encuentra la doc interna y contesta con la fuente correcta.
              </p>
              ...
              <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-[11px] text-blue-800 dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-200">
                ...
                <ol className="mt-1 list-decimal space-y-0.5 pl-4 leading-relaxed">
                  <li>Tu cuenta Atlassian debe ser miembro del site</li>
                  <li>Scopes: <code>read:confluence-content.*</code>, <code>search:confluence</code>, <code>offline_access</code></li>
                  <li>Si tienes Jira conectado en este tenant, se reusa la misma identidad Atlassian</li>
                </ol>
              </div>
          ...
          {isConnected ? (
            ...
            <>
              ...
              <button type="button" onClick={() => void handleInstall()} disabled={submitting} className="flex items-center gap-2 rounded-full bg-[#0052CC] px-5 py-2 text-[13px] font-semibold text-white transition hover:bg-[#0747A6] disabled:opacity-50">
                {submitting ? <><span className="material-symbols-outlined animate-spin text-[16px]">progress_activity</span>Redirigiendo…</> : 'Conectar con Confluence'}
              </button>
            </>
          )}
        </div>
...
export default ConfluenceConnectModal;
