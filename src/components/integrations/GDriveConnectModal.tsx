...
interface ChannelEntry { channel_id: string; resource_id: string; expiration: string }
interface Props {
  open: boolean; onClose: () => void; onChanged?: () => void;
  existing?: { google_sub?: string | null; email?: string | null; name?: string | null; scope?: string | null; channels?: ChannelEntry[] | null; capabilities?: { reads?: string[] } | null } | null;
}

const SCOPES = [
  { id: 'files',    label: 'Files · list · search · metadata',         icon: 'folder' },
  { id: 'download', label: 'Download · raw bytes · MIME-aware',         icon: 'download' },
  { id: 'export',   label: 'Export · Docs/Sheets/Slides → text/PDF',    icon: 'cloud_download' },
  { id: 'changes',  label: 'Changes feed · syncToken delta',            icon: 'sync' },
  { id: 'push',     label: 'Push notifications · channel token',        icon: 'graph_2' },
  { id: 'rag',      label: 'RAG-ready · pipeline ingest',                icon: 'menu_book' },
];

const GDriveConnectModal: React.FC<Props> = ({ open, onClose, onChanged, existing }) => {
  ...
  async function handleInstall() {
    ...
    try { const res = await authedFetch('/api/integrations/gdrive/install'); if (!res.ok) { const j = await res.json().catch(() => ({})); throw new Error(j.error || `Error ${res.status}`); } const j = await res.json(); if (j.url) window.location.href = j.url; }
  ...
  async function handleSync() {
    ...
    try { const res = await authedFetch('/api/integrations/gdrive/sync', { method: 'POST' }); const j = await res.json(); if (!res.ok) throw new Error(); setSyncResult(j); setSyncStatus('ok'); } catch { setSyncStatus('error'); }
  }
  async function handleWatch() {
    setSubmitting(true);
    try { const res = await authedFetch('/api/integrations/gdrive/watch', { method: 'POST' }); if (!res.ok) throw new Error(); onChanged?.(); }
    finally { setSubmitting(false); }
  }
  async function handleDisconnect() {
    if (!confirm('¿Desconectar Google Drive?')) return;
    setSubmitting(true);
    try { const res = await authedFetch('/api/integrations/gdrive/disconnect', { method: 'POST' }); if (!res.ok) throw new Error(); onChanged?.(); onClose(); }
    finally { setSubmitting(false); }
  }
      ...
      <div className="w-full max-w-xl overflow-hidden rounded-[28px] border border-black/5 bg-white shadow-2xl dark:border-white/10 dark:bg-[#171717]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 border-b border-black/5 px-6 py-5 dark:border-white/10">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-500 text-white shadow-sm"><IntegrationLogo id="gdrive" size={22} /></div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400">Knowledge · Google</p>
              <h2 className="text-lg font-semibold text-gray-950 dark:text-white">Google Drive</h2>
              <p className="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">Files · attachments · docs como contexto RAG (read-only)</p>
            </div>
          </div>
        ...
        <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
          {isConnected ? (
            <>
              ...
              <div className="rounded-2xl border border-black/5 bg-white p-4 dark:border-white/10 dark:bg-[#1b1b1b]">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => void handleWatch()} disabled={submitting} className="rounded-full bg-blue-500 px-3 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50">+ Watch changes</button>
                </div>
                {existing?.channels?.length ? (
                  <ul className="mt-2 space-y-1">{existing.channels.map(c => (<li key={c.channel_id} className="rounded-xl bg-gray-50 px-2.5 py-2 text-[11px] dark:bg-white/5">expira {new Date(Number(c.expiration)).toLocaleString()}</li>))}</ul>
              ...
              <div className="mt-3 rounded-2xl border border-black/5 bg-gray-50/50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => void handleSync()} disabled={syncStatus === 'syncing'} className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-[11px] dark:border-white/10 dark:bg-[#171717] dark:text-gray-200">
                    {syncStatus === 'syncing' ? '…' : syncStatus === 'ok' ? '✓ OK' : syncStatus === 'error' ? '⚠' : 'Files recientes'}
                  </button>
                </div>
                {syncResult ? (
                  <div className="mt-2 text-[11px] text-emerald-700 dark:text-emerald-300">
                    {syncResult.files_visible} files.
                    {syncResult.sample?.length ? <ul className="mt-1 space-y-0.5">{syncResult.sample.map((f: any) => <li key={f.id} className="truncate">{f.name} · {f.mime}</li>)}</ul> : null}
                  </div>
                ) : null}
              </div>
            </>
            ...
            <>
              <p className="text-sm text-gray-600 dark:text-gray-300">Read-only. El AI agent indexa tus Docs/Sheets como knowledge base; cuando un cliente pregunta algo, el agent encuentra el documento interno y contesta con la fuente.</p>
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {SCOPES.map(s => (<div key={s.id} className="flex items-center gap-2 rounded-xl border border-black/5 bg-gray-50 px-2.5 py-2 text-xs text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"><span className="material-symbols-outlined text-[14px] text-blue-500">{s.icon}</span>{s.label}</div>))}
              </div>
              <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-[11px] text-blue-800 dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-200">
                ...
                <ol className="mt-1 list-decimal space-y-0.5 pl-4 leading-relaxed">
                  <li>Scopes pedidos: <code>drive.readonly</code> + <code>drive.metadata.readonly</code></li>
                  <li>Comparte la misma cuenta Google que Calendar/Gmail si las tienes</li>
                  <li>Push channels opcionales — sin ellos pollea cada 6h</li>
                </ol>
              </div>
          ...
          {isConnected ? (
            ...
            <>
              ...
              <button type="button" onClick={() => void handleInstall()} disabled={submitting} className="flex items-center gap-2 rounded-full bg-blue-500 px-5 py-2 text-[13px] font-semibold text-white hover:bg-blue-600 disabled:opacity-50">
                {submitting ? <><span className="material-symbols-outlined animate-spin text-[16px]">progress_activity</span>Redirigiendo…</> : 'Conectar con Google Drive'}
              </button>
            </>
          )}
        </div>
...
export default GDriveConnectModal;
