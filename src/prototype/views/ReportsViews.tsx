...
// Tipos de gráfico disponibles en el editor (barra de iconos superior).
const EDITOR_TYPES: { kind: BuilderKind; label: string; path: string }[] = [
  { kind: 'kpi', label: 'Número', path: 'M4 11h2v2H4zM7 5h2v8H7zM10 8h2v5h-2z' },
  { kind: 'bar', label: 'Barras', path: 'M3 10h2v4H3zM7 6h2v8H7zM11 8h2v6h-2z' },
  { kind: 'line', label: 'Línea', path: 'M2 12l4-4 3 2 5-6' },
  { kind: 'doughnut', label: 'Circular', path: 'M8 2a6 6 0 106 6h-6z' },
  { kind: 'heatmap', label: 'Mapa de calor', path: 'M2 2h4v4H2zM8 2h4v4H8zM2 8h4v4H2zM8 8h4v4H8z' },
  { kind: 'table', label: 'Tabla', path: 'M2 3h12v3H2zM2 7h12v2H2zM2 10h12v3H2z' },
];

// Editor de gráfico a pantalla completa — cambia el tipo de gráfico, título y
// muestra el panel de configuración (métricas, ver por, segmentar). Se abre al
// pulsar "Editar" en cualquier bloque.
function ChartEditor({ item, initialKind, initialTitle, onCancel, onUpdate }: {
  ...
  const isChart = kind !== 'kpi' && kind !== 'title';
  return (
    <div className="flex flex-1 min-h-0 gap-2">
      <div className="flex-1 bg-white rounded-[12px] border border-[#e9eae6] flex flex-col min-h-0 overflow-hidden">
        {/* Header */}
        <div className="flex-shrink-0 px-6 py-3 border-b border-[#e9eae6] flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Título del gráfico" className="w-full text-[18px] font-bold text-[#1a1a1a] outline-none placeholder:text-[#c9c9c6]" />
        ...
        {/* Barra de tipos */}
        <div className="flex-shrink-0 px-6 py-2 border-b border-[#e9eae6] flex items-center gap-1">
          {EDITOR_TYPES.map(t => (
            <button key={t.kind} onClick={() => setKind(t.kind)} title={t.label}
              className={`w-8 h-8 rounded-md flex items-center justify-center ${kind === t.kind ? 'bg-[#1a1a1a] text-white' : 'text-[#646462] hover:bg-[#f3f3f1]'}`}>
              <svg viewBox="0 0 16 16" className={`w-4 h-4 ${t.kind === 'line' || t.kind === 'doughnut' ? 'fill-none stroke-current' : 'fill-current'}`} strokeWidth="1.6">{<path d={t.path} />}</svg>
            </button>
          ))}
        </div>
        {/* Preview */}
        <div className="flex-1 overflow-y-auto min-h-0 p-6">
          <div className="border border-[#e9eae6] rounded-[12px] p-4 bg-white">
            <BuilderCardBody item={{ ...item, kind, label: title || item.label }} height={340} />
          </div>
        </div>
      </div>
          ...
          {tab === 'grafico' ? (
            <>
              ...
              <div className="bg-[#fef9e7] border border-[#f5e6a8] rounded-lg p-3 text-[12px] text-[#7a6a2a] flex gap-2">
                ...
                El rango de fechas del informe anula el rango de fechas de este gráfico. Usa los filtros informe para cambiarlo.
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2"><span className="text-[13px] font-semibold text-[#1a1a1a]">Métricas <span className="text-[#646462] font-normal">1</span></span></div>
                <div className="border border-[#e9eae6] rounded-lg p-3 flex flex-col gap-2.5">
                  <p className="text-[13px] font-semibold text-[#1a1a1a]">{item.label}</p>
                  <div className="border border-[#e9eae6] rounded-md px-2.5 py-1.5 text-[12.5px] text-[#646462] flex items-center justify-between">Tiempo para cerrar y Tiempo de Inbox del bot incluido<svg viewBox="0 0 16 16" className="w-3 h-3 fill-[#646462]"><path d="M4 6l4 4 4-4z"/></svg></div>
                  <label className="flex items-center gap-2 text-[12.5px] text-[#1a1a1a]"><input type="checkbox" defaultChecked className="accent-[#3b59f6]" />Dentro del horario de atención</label>
                  <div><p className="text-[12px] font-medium text-[#1a1a1a] mb-1">Agrupación</p><div className="border border-[#e9eae6] rounded-md px-2.5 py-1.5 text-[12.5px] text-[#646462] flex items-center justify-between">Promedio<svg viewBox="0 0 16 16" className="w-3 h-3 fill-[#646462]"><path d="M4 6l4 4 4-4z"/></svg></div></div>
                ...
                <button className="text-[12.5px] text-[#646462] flex items-center gap-1 mt-2"><svg viewBox="0 0 16 16" className="w-3 h-3 fill-current"><path d="M7 3h2v4h4v2H9v4H7V9H3V7h4z"/></svg>Añadir métrica</button>
              </div>
              {isChart && (
                <div>
                  <p className="text-[13px] font-semibold text-[#1a1a1a] mb-1.5">Ver por</p>
                  <div className="border border-[#e9eae6] rounded-md px-2.5 py-1.5 text-[12.5px] text-[#1a1a1a] flex items-center gap-2 mb-2"><svg viewBox="0 0 16 16" className="w-3.5 h-3.5 fill-none stroke-[#646462]" strokeWidth="1.4"><circle cx="8" cy="8" r="6"/><path d="M8 5v3l2 1.5"/></svg>Tiempo</div>
                  ...
                  <p className="text-[13px] font-semibold text-[#1a1a1a] mt-4 mb-1.5">Segmentar por</p>
